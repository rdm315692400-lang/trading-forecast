...
class EmpiricalForecastModel:
    ...
    def metadata(self):
        return {
            "stage": "3E-1",
            "model_type": "empirical_similarity_regression_rich_features_with_fixed_baselines",
...
class ZeroChangeBaseline:
    ...
    def predict(self, features, horizon_minutes):
        h = int(horizon_minutes)
        return ModelPrediction(h, 0.0, 0.0, 0.0, 0.5, 0.5, 0, "baseline_zero_change")
...
class MomentumBaseline:
    ...
    def predict(self, features, horizon_minutes):
        ...
        return ModelPrediction(
            h, terminal,
            max(0.0, terminal) + 0.25 * magnitude,
            max(0.0, -terminal) + 0.25 * magnitude,
            p_up, 1.0 - p_up, 0, "baseline_momentum"
        )
...
class MeanReversionBaseline:
    ...
    def predict(self, features, horizon_minutes):
        ...
        return ModelPrediction(
            ...
            max(0.0, terminal) + 0.25 * magnitude,
            max(0.0, -terminal) + 0.25 * magnitude,
            p_up, 1.0 - p_up, 0, "baseline_mean_reversion"
        )
