...
class ReplayEngine:
    """
    Replays historical time sequentially.
    forecast_fn receives only a Snapshot; it never receives the store.
    """
    ...
    def run(
        ...
        end_utc: datetime,
        step_minutes: int,
        forecast_fn: Callable[[Snapshot], Any],
        model_version: str = "v13.0-foundation",
    ) -> List[FrozenForecast]:
        if step_minutes <= 0:
