...
async def minute_history(ticker, days=10):
    ...
    for x in data.get("results", []):
        ...
        out.append({"ts":ts,"dt":dt,"day":dt.date(),"minute":dt.hour*60+dt.minute,"open":x.get("o"),"high":x.get("h"),"low":x.get("l"),"close":x.get("c"),"volume":x.get("v",0),"vwap":x.get("vw")})
    return out
...
async def daily_history(ticker, calendar_days=420):
    ...
    for x in data.get("results", []):
        ...
        if min(o,h,l,c) <= 0:
            continue
        out.append({"ts":ts,"date":dt.date(),"open":o,"high":h,"low":l,"close":c,"volume":float(x.get("v") or 0),"vwap":x.get("vw")})
    return out[-250:]
