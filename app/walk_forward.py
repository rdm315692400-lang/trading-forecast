...
def evaluate(asset_id, market_day, bars, forecast_time, fitted_model):
    ...
    fid, d, x, v = forecast_snapshot(snap, fitted_model)
    if not x or not v:
        return None
...
def run_cached_walk_forward(asset_id, days, fitted_model, forecast_times=("10:30", "12:30", "14:00")):
    samples = []
    attempted_forecasts = 0
    for day in sorted(days):
        if day.weekday() >= 5:
            continue
        for hhmm in forecast_times:
            attempted_forecasts += 1
            s = evaluate(
                asset_id,
                day,
                list(days[day]),
    ...
    return {
        "ok": True,
        "asset": asset_id,
        "stage": "3G_XV_WALK_FORWARD",
        "attempted_forecasts": attempted_forecasts,
        "valid_xv_forecasts": len(samples),
        "forecast_coverage_rate": (
            len(samples) / attempted_forecasts if attempted_forecasts else None
        ),
        ...
        "holdout_used": False,
        "calibration_status": "WALK_FORWARD_VALIDATION_ONLY_NOT_CALIBRATED",
    }
