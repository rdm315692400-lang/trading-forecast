...
from .calibration import calibration_report, freeze_model_spec

router=APIRouter(prefix="/api/v14",tags=["V14"])

def _fit_stage3_model(asset_id, calendar_days=90):
    ...
    if not split["train"]:
        raise ValueError("insufficient_train_rows")
    model=EmpiricalForecastModel().fit(split["train"])
    return days,start,end,dataset,split,model

def _fit_stage3_model_before(asset_id, cutoff_day, calendar_days=90):
    """Historical forecast helper: training data must be strictly before cutoff_day."""
    days=max(1,min(int(calendar_days),730))
    end=cutoff_day-timedelta(days=1)
    start=end-timedelta(days=days-1)
    dataset=build_training_dataset(asset_id,start,end)
    rows=list(dataset.get("rows", []))
    if not rows:
        # Current dataset schema exposes examples as row dictionaries under data.
        rows=list(dataset.get("data", []))
    if not rows:
        split=chronological_split(dataset)
        rows=list(split.get("train", []))+list(split.get("validation", []))+list(split.get("holdout", []))
    if not rows:
        raise ValueError("insufficient_strictly_prior_training_rows")
    model=EmpiricalForecastModel().fit(rows)
    return model
...
async def forecast_cache(asset_id:str,market_day:str,hhmm:str="12:30"):
    ...
    try:
        ...
        model=_fit_stage3_model_before(asset_id,day,90)
        forecast_id,d,x,v=forecast_snapshot(snapshot,model)
        if not x or not v:
...
async def walk_forward(asset_id:str,end_day:str|None=None,calendar_days:int=1):
    ...
    try:
        _,_,_,_,split,model=_fit_stage3_model(asset_id,90)
        validation_days=set(split["validation_days"])
        validation_cached={d:bars for d,bars in cached.items() if d.isoformat() in validation_days}
        if not validation_cached:
            return failure(
                "walk_forward", asset_id,
                error="requested_cache_contains_no_stage3_validation_days",
                validation_days=split["validation_days"], provider_called=False,
            )
        result=run_cached_walk_forward(asset_id,validation_cached,model)
        result["evaluation_partition"]="VALIDATION_ONLY"
...
async def training_dataset(asset_id: str, calendar_days: int = 90):
    """Stage 3E/3H: build the cache-only learning dataset and return summary only."""
    asset_id = asset_id.upper().strip()
...
async def model_validation(asset_id: str, calendar_days: int = 90):
    """Stage 3H: fit TRAIN only, score VALIDATION only, keep HOLDOUT untouched."""
    asset_id = asset_id.upper().strip()
    if not get_asset(asset_id):
    ...
    try:
        ...
        for row in validation_rows:
            # Pass every model feature produced by the Stage 3D dataset.
            # The model owns the canonical feature list through its metadata.
            features = {
                name: row[name]
                for name in model.metadata()["feature_names"]
            }
        ...
        # Validation-only calibration report. Holdout is deliberately not supplied.
        validation_predictions=[]
        validation_scored_rows=[]
        for row in validation_rows:
            features={name:row[name] for name in model.metadata()["feature_names"]}
            pred=model.predict(features,int(row["horizon_minutes"]))
            if pred.status=="ok":
                validation_predictions.append(pred)
                validation_scored_rows.append(row)

        cal=calibration_report(validation_predictions,validation_scored_rows)
        frozen=freeze_model_spec(
            model,
            {
                "validation_summary": {
            ...
            version="stage3-final-candidate",
        )

        response = {
            "ok": True,
            "stage": "3H_MODEL_VALIDATION",
            "asset": asset_id,
            "requested_calendar_days": days,
            ...
            "provider_called": False,
            "firestore_write_performed": False,
            "calibration_status": "VALIDATION_ONLY_MODEL_SPEC_FROZEN_HOLDOUT_UNTOUCHED",
        }
        return response
    except Exception as exc:
