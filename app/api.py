...
async def training_dataset(asset_id: str, calendar_days: int = 90):
    """Stage 3D: build the rich-feature cache-only learning dataset and return summary only."""
    asset_id = asset_id.upper().strip()
...
async def model_validation(asset_id: str, calendar_days: int = 90):
    """Stage 3D: fit on TRAIN only and evaluate on the same VALIDATION partition. Holdout is untouched."""
    asset_id = asset_id.upper().strip()
    if not get_asset(asset_id):
    ...
    try:
        ...
        return {
            ...
            "stage_3c_reference": {
                "direction_accuracy": 0.5056714233443103,
                "mean_abs_terminal_error_pct_points": 0.3157727945658063,
                "mean_abs_upside_reach_error_pct_points": 0.26149414962715084,
                "mean_abs_downside_reach_error_pct_points": 0.2120495943388949,
                "note": "fixed prior validation result; holdout not used",
            },
